namespace Sieve.NET.Core.Sieves
{
    ...
    public class BaseSieve<TTypeOfObjectToFilter, TPropertyType> : ISieve<TTypeOfObjectToFilter, TPropertyType>
    {
        ...
        /// </summary>
        /// <exception cref="InvalidSieveValueException">
        ///     When the invalid value behavior is set to throw an exception and an
        ...
        public ICollection<TPropertyType> AcceptableValues
        {
            get
            {
                return this.GetAcceptableValues();
            }
        }
        ...
        ///     the tye of the property we're filtering on.
        /// </param>
        /// <returns>The current sieve with the additional acceptable value.</returns>
        ...
        ///     A list of values that can be converted to the same type as the property we're filtering
        ///     on.
        /// </param>
        /// <returns>The current sieve with the additional acceptable values.</returns>
        ...
        {
            List<string> acceptableValuesList = acceptableValues.ToList();
        ...
        /// </remarks>
        public ISieve<TTypeOfObjectToFilter, TPropertyType> ForProperty(
            Expression<Func<TTypeOfObjectToFilter, TPropertyType>> propertyLambda)
        {
            PropertyInfo propInfo = this.ExtractPropertyInfoFromLambda(propertyLambda);

            this.PropertyToFilter = propInfo;
        ...
        /// <remarks>This clears the previous acceptable values. To add to the list, use ForAdditionalValues().</remarks>
        /// <remarks>
        ///     This assumes that any strings passed can be converted to the property type. That responsibility is on the
        ///     user.
        /// </remarks>
        ...
        /// <summary>
        ///     Sets the behavior that the application will take when it attempts to proess a Sieve with an empty acceptable values
        ///     list.
        /// </summary>
        ...
        public ISieve<TTypeOfObjectToFilter, TPropertyType> WithEmptyValuesListBehavior(
            EmptyValuesListBehavior emptyValuesListBehavior)
        {
        ...
        /// <returns>The current sieve, set to use the given Empty Values List behavior.</returns>
        public ISieve<TTypeOfObjectToFilter, TPropertyType> WithInvalidValueBehavior(
            InvalidValueBehavior invalidValueBehavior)
        {
            this.InvalidValueBehavior = invalidValueBehavior;
        ...
        /// <example>
        ///     The sieve will use all specified separators. So, if "," and "|" are separated, "1,2|3" will become a list of 3
        ///     items -- 1, 2, and 3.
        /// </example>
            ...
            ParameterExpression parameter)
        {
            ...
            {
                BinaryExpression binaryExpression = Expression.Equal(trueConstant, trueConstant);
                Expression<Func<TTypeOfObjectToFilter, bool>> expression =
                    Expression.Lambda<Func<TTypeOfObjectToFilter, bool>>(binaryExpression, parameter);
                return expression;
            }
            if (this.EmptyValuesListBehavior == EmptyValuesListBehavior.LetNoObjectsThrough)
            {
                BinaryExpression binaryExpression = Expression.Equal(trueConstant, falseConstant);
                Expression<Func<TTypeOfObjectToFilter, bool>> expression =
                    Expression.Lambda<Func<TTypeOfObjectToFilter, bool>>(binaryExpression, parameter);
                return expression;
            }
        ...
        {
            TypeConverter converter = TypeDescriptor.GetConverter(typeof(TPropertyType));
        ...
        private PropertyInfo ExtractPropertyInfoFromLambda(
            Expression<Func<TTypeOfObjectToFilter, TPropertyType>> propertyLambda)
        {
        ...
        {
            IEnumerable<string> separators = this.GetSeparatorsOrDefault();

            List<string> parsedItems = this.ParseListOfPotentiallyAcceptableItems(separators);

            this.AddParsedItemsToPotentiallyAcceptableValues(parsedItems);

            ICollection<TPropertyType> result = this.ParsePotentiallyAcceptableValues();

            return result;
        }

        private IEnumerable<string> GetParsedValuesFromPotentiallyAcceptableParseString(
            string parseValueItem,
            IEnumerable<string> separators)
        {
            if (string.IsNullOrWhiteSpace(parseValueItem))
            {
                return new List<string>();
            }

            return
                parseValueItem.Split(separators.ToArray(), StringSplitOptions.RemoveEmptyEntries)
        ...
        private IEnumerable<string> GetSeparatorsOrDefault()
        {
            ...
            return this.addedSeparators;
        }

        private List<string> ParseListOfPotentiallyAcceptableItems(IEnumerable<string> separators)
        {
            var result = new List<string>();
            foreach (string parseValueItem in this.potentiallyAcceptableValuesToParse)
            {
                // ReSharper disable once PossibleMultipleEnumeration -- covered by tests and works fine.
                result.AddRange(this.GetParsedValuesFromPotentiallyAcceptableParseString(parseValueItem, separators));
            }
        ...
        private ICollection<TPropertyType> ParsePotentiallyAcceptableValues()
        {
            ...
            foreach (string stringItem in this.potentiallyAcceptableValues)
            {
                try
                {
                    TPropertyType convertedItem = Convert(stringItem.Trim());
                ...
                catch
                {
                    ...
                    string message = string.Format("Invalid value: {0}", stringItem);
